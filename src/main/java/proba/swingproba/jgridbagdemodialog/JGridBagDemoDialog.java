...
public class JGridBagDemoDialog extends JDialog {

    public JGridBagDemoDialog(JFrame frame) {
        super(frame, true);
        setTitle("Simple Complaints Dialog");
        setSize(500, 300);

        // Creates a panel to hold all components
        JPanel panel = new JPanel(new BorderLayout()); // BorderLayout ??? minek ez ide ...
        panel.setLayout(new GridBagLayout());
        
        // Give the panel a border gap of 5 pixels
        panel.setBorder(new EmptyBorder(new Insets(5, 5, 5, 5))); // hogy kis tavolsagot tartsanak a komponensek az ablak szeleitol
        ...
        panel.add(helpBtn, c);
        setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE); // EXIT_ON_CLOSE IllegalArgumentException-t okoz
        setVisible(true);
    }

    public static void main(String[] args) {
        new JGridBagDemoDialog(new JFrame());
    }
}